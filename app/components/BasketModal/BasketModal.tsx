...
const BasketModal = ({
    ...
    const [isFiledCheck, setIsFiledCheck] = useState<
        'error' | 'noFailed' | 'filledTime' | 'filledCode'
    >('noFailed') //флаг ошибки при проверке кода
    ...
    const checkodeSubmit = async (e: React.FormEvent) => {
        ...
        const request = await fetch('/api/auth/check-code', {
            method: 'POST',
            body: JSON.stringify({ code, fromDatabase: 'auth_codes' }),
        })
        ...
        if (responseData.checkCode === undefined) {
            ...
            (responseData.checkCode === false &&
                responseData.timer === false) ||
    ...
    const agreeRec = useWatch({ control, name: 'agreeRec' })
    ...
    const REQUIRED_FIELDS: (keyof FormValues)[] = ['name', 'phone', 'email']
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        formData.append('agreeRec', data.agreeRec ? '1' : '0')
            ...
            <div className={styles.modalOverlay} onClick={onClose}>
                ...
                >
                    <div className={styles.modal__header}>
                        <h2 className={styles.modal__title}>Создание заказа</h2>
                        <button
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className={styles.label__wrapper}>
                            <label
                                className={`${styles.phone} ${errors.phone ? styles.label_error : styles.label}`}
                            >
                                Укажите телефон
                                <Controller
                                    name="phone"
  ...
  changeAddress, */}
                        ...
                        <div className={styles.label__wrapper}>
                            ...
                            {errors.agree && (
                                <p className={styles.agreeErrmsg}>
                        ...
                        </div>
                        {
                            isCode ? (
                                !(trueCode && isFilled) && (
                                    <div className={styles.label__wrapper}>
                                        {lastCode ? (
                                            <p>{`Проверьте почту, повторная отправка через 10минут`}</p>
                                        ) : null}

                                        <label
                                            htmlFor="code"
                                            className={`${styles.index} ${styles.label}`}
                                        >
                        ...
                        {trueCode && isFilled && (
                            <button
                                className={styles.sendOrder}
                                disabled={!(trueCode && isFilled) /*isValid*/}
                                type="submit"
                            >
                                отправить
                            </button>
